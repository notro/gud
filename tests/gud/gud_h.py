...
#
class gud_drm_usb_vendor_descriptor(LittleEndianStructure):
    _fields_ = [
...
# @val: Value
#
class gud_drm_req_property(LittleEndianStructure):
    _fields_ = [
...
#
class gud_drm_req_display_mode(LittleEndianStructure):
    _fields_ = [
...
GUD_DISPLAY_MODE_FLAG_PHSYNC        = BIT(0)
GUD_DISPLAY_MODE_FLAG_NHSYNC        = BIT(1)
GUD_DISPLAY_MODE_FLAG_PVSYNC        = BIT(2)
GUD_DISPLAY_MODE_FLAG_NVSYNC        = BIT(3)
GUD_DISPLAY_MODE_FLAG_INTERLACE     = BIT(4)
GUD_DISPLAY_MODE_FLAG_DBLSCAN       = BIT(5)
GUD_DISPLAY_MODE_FLAG_CSYNC         = BIT(6)
GUD_DISPLAY_MODE_FLAG_PCSYNC        = BIT(7)
GUD_DISPLAY_MODE_FLAG_NCSYNC        = BIT(8)
GUD_DISPLAY_MODE_FLAG_HSKEW         = BIT(9)
# BCast and PixelMultiplex are deprecated
GUD_DISPLAY_MODE_FLAG_DBLCLK        = BIT(12)
GUD_DISPLAY_MODE_FLAG_CLKDIV2       = BIT(13)
...
GUD_DISPLAY_MODE_FLAG_PREFERRED     = BIT(10)
...
#
class gud_drm_req_get_connector(LittleEndianStructure):
    _fields_ = [
...
GUD_CONNECTOR_FLAGS_POLL_STATUS     = BIT(0)
GUD_CONNECTOR_FLAGS_INTERLACE       = BIT(1)
GUD_CONNECTOR_FLAGS_DOUBLESCAN      = BIT(2)
...
#
class gud_drm_req_set_buffer(LittleEndianStructure):
    _fields_ = [
        ('x', ctypes.c_uint32),
        ('y', ctypes.c_uint32),
...
#
def define_gud_drm_req_set_state(num_properties):
    class gud_drm_req_set_state(LittleEndianStructure):
        _fields_ = [
            ('mode', gud_drm_req_display_mode),
            ('format', ctypes.c_uint8),
            ('connector', ctypes.c_uint8),
            ('properties', gud_drm_req_property * num_properties),
        ]
        _pack_ = 1

    return gud_drm_req_set_state
...
GUD_ROTATION_0              = BIT(0)
GUD_ROTATION_90             = BIT(1)
GUD_ROTATION_180            = BIT(2)
GUD_ROTATION_270            = BIT(3)
GUD_ROTATION_REFLECT_X      = BIT(4)
GUD_ROTATION_REFLECT_Y      = BIT(5)
GUD_ROTATION_MASK           = (GUD_ROTATION_0 | GUD_ROTATION_90 | \
                        GUD_ROTATION_180 | GUD_ROTATION_270 | \
                        GUD_ROTATION_REFLECT_X | GUD_ROTATION_REFLECT_Y)

def property_to_str(prop):
    prop_to_name = {
    ...
    return f'{prop_to_name.get(prop, "Unknown")}'

def rotation_to_str(val):
    s = ''
    if val & GUD_ROTATION_0:
        s += '0'
    elif val & GUD_ROTATION_90:
        s += '90'
    elif val & GUD_ROTATION_180:
        s += '180'
    elif val & GUD_ROTATION_270:
        s += '270'
    if val & GUD_ROTATION_REFLECT_X:
        s += '+REFLECT_X'
    if val & GUD_ROTATION_REFLECT_Y:
        s += '+REFLECT_Y'
    return s
...
GUD_FORMATS_MAX_NUM                 = 32
# R1 is a 1-bit monochrome transfer format presented to userspace as XRGB8888
GUD_PIXEL_FORMAT_R1                 = 0x01
GUD_PIXEL_FORMAT_XRGB1111           = 0x20
GUD_PIXEL_FORMAT_RGB565             = 0x40
GUD_PIXEL_FORMAT_XRGB8888           = 0x80
GUD_PIXEL_FORMAT_ARGB8888           = 0x81

def format_to_name(fmt):
    _format_to_name = {
        GUD_PIXEL_FORMAT_R1: 'R1',
        GUD_PIXEL_FORMAT_XRGB1111: 'XRGB1111',
        GUD_PIXEL_FORMAT_RGB565: 'RGB565',
        GUD_PIXEL_FORMAT_XRGB8888: 'XRGB8888',
        GUD_PIXEL_FORMAT_ARGB8888: 'ARGB8888',
    }

    return _format_to_name.get(fmt, f'{fmt:02x}')

def name_to_format(name):
    _name_to_format = {
        'R1': GUD_PIXEL_FORMAT_R1,
        'XRGB1111': GUD_PIXEL_FORMAT_XRGB1111,
        'RGB565': GUD_PIXEL_FORMAT_RGB565,
        'XRGB8888': GUD_PIXEL_FORMAT_XRGB8888,
        'ARGB8888': GUD_PIXEL_FORMAT_ARGB8888,
    }

    return _name_to_format.get(name, None)
...
GUD_CONNECTOR_PROPERTIES_MAX_NUM    = 32
...
GUD_CONNECTOR_TV_MODE_NAME_LEN      = 16
GUD_CONNECTOR_TV_MODE_MAX_NUM       = 16
...
GUD_CONNECTOR_STATUS_CONNECTED_MASK = 0x03
...
# Used by tests to issue an unsupported request
GUD_REQ_UNSUPPORTED                         = 0xca
